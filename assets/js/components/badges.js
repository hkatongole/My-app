...
export function leagueBadge(name, { size = 'sm' } = {}) {
  ...
  const logoUrl = logoRepository.getLeague(name);
  const hue = hashHue('league:' + name);
  
  if (!logoUrl) {
    return `<span class="badge badge--${size} badge--league" style="--badge-hue:${hue}" title="${escapeHtml(name)}">${escapeHtml(initials(name))}</span>`;
  }
  ...
  return `
    <span class="badge-wrap badge-wrap--${size}">
      <img src="${escapeHtml(logoUrl)}" alt="${escapeHtml(name)}" title="${escapeHtml(name)}"
           class="badge-img badge-img--${size}" loading="lazy"
           onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';" />
      <span class="badge badge--${size} badge--league" style="--badge-hue:${hue}; display:none;">${escapeHtml(initials(name))}</span>
    </span>
  `.trim();
}
