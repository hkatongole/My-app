...
async function seasonFromQuery(league, query) {
  return query?.season || await leagueRepository.latestSeason(league);
}
...
export async function renderLeagueResults({ league, query }) {
  ...
  const page = Number(query.page || 1);
  const seasons = await leagueRepository.seasonsFor(league);
  const { rows, total, totalPages } = await leagueRepository.resultsPage(league, { season, page, pageSize: 20 });
