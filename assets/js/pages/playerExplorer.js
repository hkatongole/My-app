...
export async function renderPlayerDirectory({ query }) {
  ...
  const { rows, total, totalPages } = await playerRepository.directory({
    league, season: season === 'latest' ? null : season, position, search, page, pageSize: 30,
  });
  const leagues = await playerRepository.distinctLeagues();
  const positions = await playerRepository.distinctPositions();
