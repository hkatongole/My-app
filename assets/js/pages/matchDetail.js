...
export async function renderMatchDetail({ id, query = {} }) {
  ...
  const { match, prediction, odds, weather, injuries, market, h2h_matches } = bundle;
  const asOf = [prediction?.predicted_at, prediction?.evaluated_at, match.scraped_at].filter(Boolean).sort().reverse()[0];

  const homeStats = await teamRepository.statsFor(match.home_team, match.season);
  const awayStats = await teamRepository.statsFor(match.away_team, match.season);
  ...
  if (tab === 'comparison') {
    ...
    body = (prediction && prediction.h2h_total != null) || (h2h_matches && h2h_matches.length > 0)
      ? h2hSection(prediction, h2h_matches)
      : `<div class="empty-state"><p>No head-to-head data on record for this fixture.</p></div>`;
  } else if (tab === 'markets') {
    body = market 
      ? marketsSection(market)
      : `<div class="empty-state"><p>No market predictions on record for this match.</p></div>`;
...
function h2hSection(p, h2h_matches) {
  let table = '';
  if (h2h_matches && h2h_matches.length > 0) {
    table = `
      <table class="data-table data-table--compact" style="margin-top: 16px;">
        <thead><tr><th>Date</th><th>Competition</th><th>Home</th><th>Score</th><th>Away</th></tr></thead>
        <tbody>
          ${h2h_matches.map(m => `
            <tr>
              <td>${m.match_date}</td>
              <td>${m.league} ${m.season ? `(${m.season})` : ''}</td>
              <td style="font-weight: ${m.home_score > m.away_score ? 'bold' : 'normal'}">${m.home_team}</td>
              <td>${m.home_score} - ${m.away_score}</td>
              <td style="font-weight: ${m.away_score > m.home_score ? 'bold' : 'normal'}">${m.away_team}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
  ...
  return `
    <div class="panel">
      ...
      <p>${p?.h2h_total ?? 0} previous meetings &middot; Home win rate ${p?.h2h_home_rate != null ? formatPct(p.h2h_home_rate) : '\u2014'} &middot; Avg goals ${p?.h2h_avg_goals ?? '\u2014'}</p>
...
function marketsSection(market) {
  return `
    <div class="panel">
      <h3>Match Markets</h3>
      <div class="stat-grid">
        <div class="stat-box">
          <span class="stat-box__label">Expected Goals (Home - Away)</span>
          <span class="stat-box__value">${market.expected_home ?? '\u2014'} - ${market.expected_away ?? '\u2014'}</span>
        </div>
        <div class="stat-box">
          <span class="stat-box__label">Both Teams to Score</span>
          <span class="stat-box__value">Yes: ${formatPct(market.btts_yes)} / No: ${formatPct(market.btts_no)}</span>
        </div>
        <div class="stat-box">
          <span class="stat-box__label">Double Chance</span>
          <span class="stat-box__value">1X: ${formatPct(market.dc_1x)} / 12: ${formatPct(market.dc_12)} / X2: ${formatPct(market.dc_x2)}</span>
        </div>
        <div class="stat-box">
          <span class="stat-box__label">To Score</span>
          <span class="stat-box__value">Home: ${formatPct(market.home_to_score)} / Away: ${formatPct(market.away_to_score)}</span>
        </div>
      </div>

      <h3 style="margin-top: 24px;">Over / Under Goals</h3>
      <table class="data-table data-table--compact">
        <thead><tr><th>Market</th><th>Over</th><th>Under</th></tr></thead>
        <tbody>
          ${market.over_05 != null ? `<tr><td>0.5 Goals</td><td>${formatPct(market.over_05)}</td><td>${formatPct(market.under_05)}</td></tr>` : ''}
          ${market.over_15 != null ? `<tr><td>1.5 Goals</td><td>${formatPct(market.over_15)}</td><td>${formatPct(market.under_15)}</td></tr>` : ''}
          ${market.over_25 != null ? `<tr><td>2.5 Goals</td><td>${formatPct(market.over_25)}</td><td>${formatPct(market.under_25)}</td></tr>` : ''}
          ${market.over_35 != null ? `<tr><td>3.5 Goals</td><td>${formatPct(market.over_35)}</td><td>${formatPct(market.under_35)}</td></tr>` : ''}
        </tbody>
      </table>
      
      ${market.best_bet_outcome ? `
      <h3 style="margin-top: 24px;">Best Bets</h3>
      <div class="stat-grid">
        <div class="stat-box">
          <span class="stat-box__label">Top Pick</span>
          <span class="stat-box__value">${market.best_bet_outcome} (${formatPct(market.best_bet_prob)})</span>
        </div>
        ${market.second_best_bet_outcome ? `
        <div class="stat-box">
          <span class="stat-box__label">Secondary Pick</span>
          <span class="stat-box__value">${market.second_best_bet_outcome} (${formatPct(market.second_best_bet_prob)})</span>
        </div>
        ` : ''}
      </div>
      ` : ''}
