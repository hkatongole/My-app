...
export async function renderTeamResults({ team, query }) {
  ...
  const seasons = await teamRepository.seasonsFor(team);
  const { rows, total, totalPages } = await teamRepository.resultsFor(team, { season, opponent, result, page, pageSize: 20 });
