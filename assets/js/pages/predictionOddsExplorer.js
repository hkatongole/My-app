...
export async function renderPredictionOddsExplorer({ query }) {
  ...
  let body;
  if (tab === 'match_odds') body = await renderMatchOddsTab(query, isMember);
  else if (tab === 'fortebet_odds') body = await renderFortebetOddsTab(query, isMember);
  else body = await renderPredictionsTab(query, isMember);
...
async function renderPredictionsTab(query, isMember) {
  ...
  const { rows, total, totalPages } = await predictionRepository.filterPredictions({
  ...
  const markets = await predictionRepository.distinctMarkets();
  const confidences = await predictionRepository.distinctConfidences();
...
async function renderMatchOddsTab(query, isMember) {
  const page = Number(query.page || 1);
  const league = query.league || null;
  const { rows, total, totalPages } = await oddsRepository.matchOddsQuery({ league, page, pageSize: 25 });
  const leagues = await oddsRepository.distinctMatchOddsLeagues();
...
async function renderFortebetOddsTab(query, isMember) {
  const page = Number(query.page || 1);
  const league = query.league || null;
  const { rows, total, totalPages } = await oddsRepository.fortebetOddsQuery({ league, page, pageSize: 25 });
  const leagues = await oddsRepository.distinctFortebetLeagues();
