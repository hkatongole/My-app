...
export async function renderTeamDirectory({ query }) {
  ...
  const rows = await teamRepository.directory({ league, season: season === 'latest' ? null : season });
  ...
  const allLeagues = league ? await teamRepository.directory({ season: season === 'latest' ? null : season }) : rows;
