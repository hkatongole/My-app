...
export async function renderModelPerformance({ query }) {
  ...
  let body;
  if (tab === 'calibration') body = await calibrationTab();
  else if (tab === 'weights') body = await weightsTab();
  else body = await accuracyTab();
...
async function accuracyTab() {
  const rows = await predictionRepository.accuracyOverTime({});
  if (!rows.length) {
...
async function calibrationTab() {
  const rows = await predictionRepository.calibrationCurve({});
  if (!rows.length) {
...
async function weightsTab() {
  const rows = await predictionRepository.engineWeightHistory();
  if (!rows.length) {
