...
class MatchRepository extends BaseRepository {
  ...
  upcomingFixtures(fromDateStr, { league = null, limit = 20 } = {}) {
    if (!this.exists()) return [];
    const predCols = storage.availableColumns('prediction_log', [
      'consensus_outcome',
      'consensus_home_prob',
    ...
    const sql = `
      ...
      ${joinClause}
      WHERE ${where.join(' AND ')}
      ORDER BY m.match_date ASC, m.start_time ASC, m.league ASC
      LIMIT ?
    `;
    params.push(limit);
    return storage.all(sql, params);
  ...
  detailBundle(matchId) {
    ...
    if (storage.hasTable('market_predictions')) {
      bundle.market = storage.get(`SELECT * FROM market_predictions WHERE match_id = ? LIMIT 1`, [match.id]);
    }
    if (storage.hasTable('historical_results')) {
      bundle.h2h_matches = storage.all(
