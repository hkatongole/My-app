...
class LogoRepository {
  ...
  _ensureLoaded() {
    ...
    try {
      ...
      if (storage.hasTable('league_logos')) {
        const lRows = storage.all('SELECT league, logo_url FROM league_logos WHERE logo_url IS NOT NULL');
        for (const r of lRows) {
          if (r.league) this._cache.set('league:' + r.league, r.logo_url);
        }
      }
    } catch (err) {
  ...
  /** Returns the logo URL for a league, or null if none is on record. */
  getLeague(league) {
    if (!league) return null;
    this._ensureLoaded();
    return this._cache.get('league:' + league) || null;
  }
}
